...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'SalesItem.item'
        db.delete_column(u'sales_salesitem', 'item_id')

        # Adding field 'SalesItem.delivery_note_item'
        db.add_column(u'sales_salesitem', 'delivery_note_item',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['sales.DeliveryNoteItem'], null=True, blank=True),
                      keep_default=False)

    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'SalesItem.item'
        raise RuntimeError("Cannot reverse this migration. 'SalesItem.item' and its values cannot be restored.")
        # Deleting field 'SalesItem.delivery_note_item'
        db.delete_column(u'sales_salesitem', 'delivery_note_item_id')
