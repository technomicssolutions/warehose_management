...
function SalesDNController($scope, $element, $http, $timeout, share, $location) {
    ...
    $scope.validate_sales = function() {
        ...
        if($scope.sales.sales_invoice_date == '') {
            ...
            return false;
        } else if($scope.sales.salesman =='select'){
            $scope.validation_error = "Please choose Salesman";
            return false;
        } else if($scope.sales.sales_items.length == 0){
            $scope.validation_error = "Choose Item";
            return false;
        } else if($scope.sales.customer =='select'){
            $scope.validation_error = "Please Choose Customer";
            return false;
    ...
    $scope.getItems = function(parameter){
        ...
        if ($scope.sales.salesman == 'select' || $scope.sales.salesman == '') {
            $scope.salesman_validation_error = 'Please choose sales man';
            $scope.items = [];
            $scope.sales.sales_items = [];
        } else {
            $scope.salesman_validation_error = '';
            if(parameter == 'item_code')
                var param = $scope.item_code;
            else if(parameter == 'item_name')
                var param = $scope.item_name;
                ...
                var param = $scope.barcode;
            $http.get('/sales/delivery_note_items/?'+parameter+'='+param+'&salesman='+$scope.sales.salesman).success(function(data)
            {
                $scope.selecting_item = true;
                $scope.item_selected = false;
                $scope.items = data.items;
...
function EditSalesInvoiceController($scope, $element, $location, $http){
    ...
    $scope.add_invoice = function(invoice) {
        ...
        $scope.invoice_details.balance = invoice.balance;
        $scope.invoice_details.balance_payment = invoice.balance;
        ...
        if(invoice.items.length > 0){
            for(var i=0; i< invoice.items.length; i++){
                var selected_item = {
                    ...
                    'dis_amt': invoice.items[i].discount,
    ...
    $scope.calculate_net_total_amount = function() {
        var total_amount = 0
        for(var i=0; i< $scope.invoice_details.sales_items.length; i++){
            total_amount = (parseFloat($scope.invoice_details.sales_items[i].net_amount)).toFixed(2);
        }
    ...
    $scope.calculate_net_amount_sale = function(item) {
        ...
        if(item.qty_sold != '' && item.unit_price != ''){
            ...
            var amount = ((parseFloat(item.qty_sold)*parseFloat(item.unit_price))-parseFloat(item.dis_amt)).toFixed(2);
    ...
    $scope.calculate_net_discount_sale = function(){
        ...
        for(i=0; i<$scope.invoice_details.sales_items.length; i++){
            ...
            net_discount = net_discount + parseFloat($scope.invoice_details.sales_items[i].dis_amt);
    ...
    $scope.calculate_discount_amt = function(item) {
        ...
        if((item.dis_percentage != '' || item.dis_percentage != 0) && (item.unit_price != '' || item.unit_price != 0)) {
            ...
            item.net_amount = (parseFloat(item.net_amount) - parseFloat(amount)) + parseFloat(item.amt);
            $scope.invoice_details.balance = parseFloat($scope.invoice_details.grant_total) - parseFloat($scope.invoice_details.paid_amount);
        }
        $scope.calculate_net_total_sale();
